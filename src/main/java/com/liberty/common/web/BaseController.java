...
public class BaseController extends Controller {
    ...
    public void handleInclude(List<Kline> klines, Stroke stroke) {
        ...
        for (int i = 0; i < size - 5; i++) {
            if ("0".equals(strokes.get(i).getDirection())) {// 第一笔向上
                ...
                if (strokes.get(i + 3).getMin() < premax && strokes.get(i + 3).getMax() > premax) {// 笔破坏,由上转下
                    ...
                    if (Stroke.dao.overlap(strokes.get(i + 3), strokes.get(i + 4), strokes.get(i + 5)) > 0) {
                        i = i + 1;
                        continue;
                    } else {// 笔破坏最终成立
...
//							lines.addAll(tmpLines);
    ...
    public List<Shape> handleShapes(List<Kline> klines, Stroke inStroke,boolean deleteLastStroke) {
        ...
        for (int i = 0; i < klines.size() - 2; i++) {
            ...
            if (Shape.dao.isHighShape(kline0, kline1, kline2)) {// 顶分
                shape = new Shape().setDate(kline1.getDate()).setType(Shape.SHAPE_TYPE_HIGH).setMax(kline1.getMax());
                endIndex = i;
            } else if (Shape.dao.isLowShape(kline0, kline1, kline2)) {// 底分
                shape = new Shape().setDate(kline1.getDate()).setType(Shape.SHAPE_TYPE_LOW).setMin(kline1.getMin());
                endIndex = i;
            } else {
                continue;
            }
     ...
     * @return
     */
    public List<Stroke> handleStrokes(List<Shape> shapes, int currencyId, String type) {
    ...
    public List<Stroke> processStrokes_new(List<Kline> klines, Stroke inStroke) {
        ...
        List<Stroke> strokes = handleStrokes(shapes, currencyId, type);
     ...
     * @return
     */
    private boolean innerExtremeCheck(int startIndex, int endIndex, List<Kline> klines, Shape preShape, Shape shape) {
        if ("0".equals(shape.getType())) {
            // 向上笔
